...
- Image tests never touch a terminal: stub `terminal.writer` (captures the exact escape bytes), `terminal.detect`, `terminal.is_tmux`, `terminal.is_remote`, `terminal.tmux_offset`, and `terminal.cell_size` as in `tests/preview/test_image.lua`; `terminal._reset()`, `kitty._reset()`, and `image._reset()` clear module caches and augroups between cases
...
- Image preview (`image/terminal.lua`, `image/kitty.lua`, `image/convert.lua`, `preview/image.lua`) is a self-contained Kitty graphics client; `eda.image.kitty` mirrors `vim.ui.img` (`set` / `get` / `del`) so it can be replaced once that API stabilizes; the one extension is `crop` (a source rectangle, Kitty keys `x,y,w,h`), which `vim.ui.img` has no equivalent for and a migration would have to drop or emulate. Placement is cursor-positioned: `ESC 7`, cursor move, `a=p`, `ESC 8` in a single write, because tmux homes the outer cursor after every passthrough chunk. Screen coordinates are `nvim_win_get_position` (outer border corner, already in tabline-inclusive screen rows) + border + tmux offset (`pane_top` plus the top status line height, `pane_left`) + 1
- `terminal.is_remote()` checks the SSH variables of the process and, inside tmux, `tmux show-environment SSH_CONNECTION`: tmux's `update-environment` rewrites the session value on every attach, so a later SSH attach to a locally started server is detected. The answer is cached one tick like `tmux_offset`; the last attach wins when several clients are attached
...
- Images go through `convert.to_png` when they are not PNG or exceed the preview pane's pixel size (`convert.bound_for`: window cells × `terminal.cell_size()`, each axis rounded up to 256px; direct transmission also caps each axis at `convert.max_side` = 2048px, file-path transmission passes `math.huge`; the bound is part of the cache key). Without `magick`, PNGs up to 2048px on a side are shown unconverted. `fit_cells` always decides both cell axes: WezTerm fills an omitted axis from the image's pixel size instead of deriving it from the given one, so a single-axis placement of a downscaled image spills out of the window. Terminals stretch the image to the whole-cell box ignoring aspect ratio, so `fit_cells` rounds the short axis (floor or ceil, whichever loses less), derives the long axis from it, and sends a centered `crop` that matches the box aspect; the residual crop or shrink is at most half a cell of the short axis, which is only visible for images spanning fewer than ~3 cells on one side
...
- Workload scripts write JSON samples (`EDA_BENCH_DIR` fixture, `EDA_BENCH_OUTPUT` path); fixtures and invocations are in `benchmarks/README.md`: `toggle.lua` (directory toggle), `dirty-toggle.lua` (toggle with unsaved edits), `edit-resync.lua` (icon resync after `dd`/undo), `git-render.lua` (full render and toggle with Git status), `git-refresh.lua` (watcher refresh with unchanged Git status), `git-burst.lua` (status during save bursts), `symlinks.lua` (symlink scans)
- Wall-clock times on a loaded machine vary by 2x between runs: compare revisions by alternating base and change in the same session (a detached `git worktree` of the base as cwd), and read `cpu_ms` (process CPU time) alongside `total_ms`
- Find hotspots before optimizing: wrap the measured loop in `require("jit.p").start("Fi1", out)` / `require("jit.p").stop()` (`l` for per-line, `v` for VM state). A `string.find`/`match` with a pattern is not compiled by LuaJIT and drops the surrounding loop to the interpreter; per-row byte loops stay compiled
