...
| `D`     | mark_bulk_delete   | Delete all marked nodes       |
...
- **duplicate** — Duplicate the file under the cursor with a prompted name.
- **cut** — Cut selected nodes (cursor or visual selection) to the register.
- **copy** — Copy selected nodes to the register.
- **paste** — Paste from the register into the directory under the cursor.
  Appends `_copy` suffix on name collision.
- **mark_toggle** — Toggle mark on the current node and move cursor down.
- **mark_bulk_delete** — Delete all marked nodes with confirmation.
- **mark_bulk_move** — Move all marked nodes to a prompted directory.
  *No default mapping.*
